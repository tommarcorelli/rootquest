...
Statut actuel : **v1.10 fonctionnelle** — 23 machines en 3 tiers + import/export JSON de box perso (tier « Personnalisées ») + défi du jour/surprends-moi, hub par tiers + scorecard, **profil opérateur + 8 succès**, terminal réaliste (pipes + `grep`/`ps`/`env`/`man`/autocomplétion/`tee`/`john`/`mount`/`showmount`/**éditeur `nano` plein écran**), **mode Blue Team**, **mode explication** (solution commentée pas-à-pas, togglable), **carte « preuve de root » partageable** (PNG téléchargeable, thème dynamique), **5 thèmes**, **son**, **historique persistant + `Ctrl+R`**, **timer speedrun + meilleurs temps**, **accessibilité clavier/ARIA**, **responsive mobile**, **offline-first** (polices auto-hébergées, zéro dépendance externe), bilingue EN/FR. Tests : harnais Node 35/35 + Playwright, CI GitHub. Déploiement GitHub Pages prêt (à activer une fois).
